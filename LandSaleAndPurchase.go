...
type PropertyByAdhaar struct {
	OwnerDetails    Owner      `json:"ownerDetails"` //generated by blockchain
...
func (t *SimpleChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "listPropertyByAdhaar" {

		return listPropertyByAdhaar(stub, args)
	}
...
func listPropertyByAdhaar(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {

	propertyByAdhaar := PropertyByAdhaar{}

	var ownerDetailsBytes []byte
	var propertyDetailsBytes []byte
	var propertyByAdhaarBytes []byte
	var err error
	var ownerDetails Owner
	var propertyDetails []Property

	ownerDetailsBytes, err = searchOwnerByAdhaar(stub, args)
	json.Unmarshal(ownerDetailsBytes, &ownerDetails)

	fmt.Println("Owner details got ", string(ownerDetailsBytes))

	if err != nil {
		return nil, errors.New("Error unmarshalling owner details")
	}

	propertyByAdhaar.OwnerDetails = ownerDetails

	ownerIdStr := strconv.Itoa(ownerDetails.Id)

	args[0] = ownerIdStr

	propertyDetailsBytes, err = listRegisteredPropertiesByOwnwer(stub, args)
	json.Unmarshal(propertyDetailsBytes, &propertyDetails)

	fmt.Println("propertyDetails details got ", string(propertyDetailsBytes))

	if err != nil {
		return nil, errors.New("Error unmarshalling owner details")
	}

	propertyByAdhaar.PropertyDetails = propertyDetails

	propertyByAdhaarBytes, err = json.Marshal(propertyByAdhaar)

	if err != nil {
		return nil, errors.New("Error unmarshalling owner details")
	}

	return propertyByAdhaarBytes, nil
